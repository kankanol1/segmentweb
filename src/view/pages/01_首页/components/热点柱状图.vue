<template>
  <div style="width: 100%; height: 100%;">
    <div id="ele1593399342963" style="width: 100%; height: 100%;">
    </div>
  </div>
</template>
    ...
    data() {
      return {
        ...
        option: (xAxis, validData, unvalidData, validProportion) => ({
          ...
          yAxis: [
            {type: 'value', name: '采集总量(万条)', min: 0, interval: 700, axisLabel: {formatter: '{value}'}},
    ...
    methods: {
      draw() {
        ...
        this.content.sort((a, b) => a.day - b.day).forEach(it => {
          xAxis.push(it.day);
          validData.push(it.imsi / 10000);
          unvalidData.push((it.imsiFull > it.imsi ? it.imsiFull - it.imsi : 0) / 10000);
          validProportion.push(((it.imsiFull > it.imsi ? it.imsi / it.imsiFull : 100) * 100).toFixed(3));
        });
        let echarts = require('echarts');
        let rdChart = echarts.init(document.getElementById('ele1593399342963'));
        rdChart.setOption(this.option(xAxis, validData, unvalidData, validProportion));
        this.$rdChart = rdChart;
        window.addEventListener('resize', this.resize);
      },
      resize() {
        this.$rdChart && this.$rdChart.resize();
      },
    },
  }
</script>
