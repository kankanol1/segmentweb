<template>
  <div class="img-box">
    <div class="img-box-item img-box-title">
      <div>
        <Button style="width: 100px" type="primary" @click="selectFile">选择文件</Button>
        <input
          hidden
          class="img-button"
          accept="image/png"
      ...
      <div>
        <Button style="width: 100px" type="primary" @click="submitFun" :loading="modal1">{{modal1?"处理中...":"图片处理"}}</Button>
      </div>
    </div>
    <Row>
      ...
      <Col span="12" class="img-boxs" style="position: relative">
        <div>
          <img class="img"  v-if="src2" id="uploadFile" :src="src2" alt="">
        </div>
        <ul class="percent-ul" style="position: absolute;right:0 !important;">
          <li v-for="item in nums">
            <span class="color" :style="{backgroundColor:'rgba('+item[0][0]+','+item[0][1]+','+item[0][2]+',1)'}"></span>
